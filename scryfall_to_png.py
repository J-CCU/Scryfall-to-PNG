...
class ScryfallBulkGUI(tk.Tk):
    ...
    def load_bulk(self):
        try:
            ...
            # FIX: Guarantee rebuild on fresh machine
            if meta is None:
                self.log_message("No metadata found. Forcing full rebuild.")

            if meta is None or not bulk_file_is_fresh(BULK_FILENAME, meta):
    ...
    def build_template_row(self, src_row: dict) -> dict:
        ...
        out["Continue selling when out of stock"] = (
            "CONTINUE" if inv_policy.lower() == "continue" else ""
        )
